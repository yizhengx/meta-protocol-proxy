...
bool RedisHeader::decode(Buffer::Instance& buffer) {
    ...
    command = static_cast<RedisCommand>(buffer.peekBEInt<uint8_t>(1));
...
bool RedisHeader::encode(Buffer::Instance& buffer) {
    ...
    buffer.writeBEInt<uint16_t>(vbucket_id);
...
// bool RedisRequestHeader::decode(Buffer::Instance& buffer) {
//     if (buffer.length() < MEMCACHED_HEADER_SIZE) {
//         return false; // Need more data
//     }
...
// bool RedisResponseHeader::encode(Buffer::Instance& buffer) {
//     buffer.writeBEInt<uint8_t>(magic);
//     buffer.writeBEInt<uint8_t>(static_cast<uint8_t>(command));
